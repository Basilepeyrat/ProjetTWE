...
function mkLiens($tabData,$champLabel, $champCible, $urlBase=false, $nomCible="")
{
	...
	if (!is_array($tabData) || count($tabData) == 0) return;

	echo "<ul class=\"liens\">\n";
	foreach ($tabData as $data)
	{
		$label = $data[$champLabel];
		$cible = $data[$champCible];

		if ($urlBase === false)
		{
			// URL directe : la valeur du champ cible EST l'URL
			$href = $cible;
		}
		else
		{
			// On construit l'URL en ajoutant $nomCible=$cible à urlBase
			$href = $urlBase . "&" . $nomCible . "=" . urlencode($cible);
		}
